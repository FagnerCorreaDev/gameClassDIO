/*
## Objetivo:

Crie uma classe genérica que represente um herói de uma aventura e que possua as seguintes propriedades:

- nome
- idade
- tipo (ex: guerreiro, mago, monge, ninja )

além disso, deve ter um método chamado atacar que deve atender os seguintes requisitos:

- exibir a mensagem: "o {tipo} atacou usando {ataque}")
- aonde o {tipo} deve ser concatenando o tipo que está na propriedade da classe
- e no {ataque} deve seguir uma descrição diferente conforme o tipo, seguindo a tabela abaixo:

se mago -> no ataque exibir (usou magia)
se guerreiro -> no ataque exibir (usou espada)
se monge -> no ataque exibir (usou artes marciais)
se ninja -> no ataque exibir (usou shuriken)

## Saída

Ao final deve se exibir uma mensagem:

- "o {tipo} atacou usando {ataque}"
ex: mago atacou usando magia
guerreiro atacou usando espada
*/

class hero {
    constructor(nome, idade, tipo) {
        this.nome = nome
        this.idade = idade
        this.tipo = tipo
    }
    attack() {
        if (this.tipo === "mago") {
            console.log("O " + this.tipo + " " + this.nome + " atacou usando magia")
        } else if (this.tipo === "guerreiro") {
            console.log("O " + this.tipo + " " + this.nome + " atacou usando espada")
        } else if (this.tipo === "monge") {
            console.log("O " + this.tipo + " " + this.nome + " atacou usando artes marciais")
        } else {
            console.log("O " + this.tipo + " " + this.nome + " atacou usando shuriken")
        }
    }
}
let mago = new hero("Spellman", 289, "mago")
let guerreiro = new hero("Kreator", 120, "guerreiro")
let monge = new hero("Jackie Xeng", 39, "monge")
let ninja = new hero("Kazuya", 25, "ninja")

mago.attack()
guerreiro.attack()
monge.attack()
ninja.attack()